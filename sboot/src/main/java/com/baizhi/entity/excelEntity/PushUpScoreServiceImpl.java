...
import static com.baizhi.service.Impl.PullUpScoreServiceImpl.getScoreIndex;

public class PushUpScoreServiceImpl implements SourceScoreService {
    @Override
    public Integer getSourceScore(int age, Integer gender, Integer number) {
        int score = 0;
        int[] sourceArray = new int[0];
        //年龄下标
        int ageIndex = getIndex(PushUpConsts.AGE_ARRAY,age);
        if(gender==0){
            sourceArray =PushUpConsts.SOURCE_ARRAY_0[ageIndex];
        }else if(gender==1){
            sourceArray=PushUpConsts.SOURCE_ARRAY_1[ageIndex];
        }
        //成绩下标
        int sourceIndex = getScoreIndex(sourceArray,number);
        //分数
        score = PushUpConsts.SCORE_ARRAY[sourceIndex];
        //成绩上限
    ...
    public static void main(String[] args) {
        int source=29;
        int sourceEnd=20;
        int score = 100+(source-sourceEnd)/2;
    }
}
